...
df = spark.readStream.format("kafka") \
    ...
    .option("subscribe", "bitcoin-price-update") \
...
parsed_df = df.select(from_json(col("value"), schema).alias("data")).select("data.*") \
    .select("time.updated", "bpi.USD.rate_float")
...
def write_to_mysql(row):
    ...
    query = "INSERT INTO updateBitcoin(utime, rate) VALUES (%s, %s)"

    # Execute the SQL query with the row data
    cursor.execute(query, (row.updated, row.rate_float))
